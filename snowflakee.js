...
function updateCanvasSize() {
	screen = {
		...
		c: [0.5 * window.innerWidth, 0.5 * window.innerHeight],
		r: Math.max(100, 0.3 * Math.min(window.innerWidth, window.innerHeight))
	};
...
function onMove([x, y] = cursorPosition) {
	if (drawingAllowed) {
		...
		mouseDistance = Math.sqrt(
			Math.pow(mousePoint[0], 2) + Math.pow(mousePoint[1], 2)
		);
...
function randomPointOnBaseAngle() {
	...
	return [r * Math.cos(baseAngle), r * Math.sin(baseAngle)];
}
